...
const buildUniquenessData = (values, mode = "classical", maxPoints = 60) => {
  ...
  const raw = values.map((v, i) => {
    const key = mode === "classical" ? v.toFixed(4) : `${i}:${v.toFixed(6)}`;
    seen.add(key);
 ...
 */
const buildRepetitionFrequency = (values, buckets = 20) => {
  const freq = new Map();
  for (let i = 0; i < buckets; i++) {
    const label = `${Math.floor(i * (100 / buckets))}`;
    freq.set(label, 0);
  }
  values.forEach((v) => {
    const idx = Math.min(Math.floor(v * buckets), buckets - 1);
    const label = `${Math.floor(idx * (100 / buckets))}`;
...
const exportCSV = (values, filename = "rng-data.csv") => {
  const rows = ["index,value,toFixed4"].concat(
    values.map((v, i) => `${i + 1},${v},${v.toFixed(4)}`)
  );
...
// ─────────────────────────────────────────────
const ClassicalPanel = ({ speed, onValue }) => {
  ...
  const generate = useCallback(() => {
    ...
    setLatest(v);
    setValues((prev) => [...prev.slice(-299), v]);
    onValue && onValue(v);
  }, [onValue]);
  ...
  const repetitionData = buildRepetitionFrequency(values, 20);
  ...
  const displayValue = (v) => {
    ...
    if (viewMode === "bits") return (Math.floor(v * 256)).toString(2).padStart(8, "0");
    return v.toFixed(8);
  };
    ...
    <GlassCard className="flex flex-col gap-4 p-5 h-full" glow={isRunning}>
      ...
      <motion.div layout className="rounded-xl p-4 text-center"
        ...
        <p className="font-mono text-xs mt-1" style={{ color: COLORS.textDim }}>
          {viewMode === "decimal" ? "float ∈ [0, 1)" : viewMode === "binary" ? "16-bit binary" : "8-bit stream"}
        </p>
      </motion.div>
          ...
          {Array.from({ length: 20 }).map((_, i) => {
            ...
            return (
              <motion.div key={i} title={v?.toFixed(4)}
                ...
                style={{
                  height: 18,
                  background: v !== undefined ? `rgba(${Math.floor(v * 180 + 40)},0,0,0.9)` : "rgba(20,5,5,0.3)",
...
// ─────────────────────────────────────────────
const QuantumPanel = ({ speed, onBit, onValue }) => {
  ...
  const generate = useCallback(() => {
    ...
    setQValues((prev) => [...prev.slice(-299), val]);
    setTriggerKey((k) => k + 1);
    onBit && onBit(bit);
    onValue && onValue(val);
  }, [onBit, onValue]);
  ...
  const repetitionData = buildRepetitionFrequency(qValues, 20);
...
export default function App() {
  ...
  const handleClassicalValue = useCallback((v) => {
    setClassicalHistory((prev) => [...prev.slice(-299), v]);
  }, []);

  const handleQuantumValue = useCallback((v) => {
    setQuantumHistory((prev) => [...prev.slice(-299), v]);
  }, []);
  ...
  return (
    <div className="min-h-screen relative" style={{ background: COLORS.bg, color: COLORS.text }}>
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        {/* ── MAIN CONTENT GRID ── */}
        <div className="flex-1 p-4 md:p-6" style={{ display: "grid", gap: "1.25rem" }}>

          {/* ROW 2: Side-by-side panels */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-start">
            <motion.div initial={{ opacity: 0, x: -28 }} animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.5, delay: 0.2 }}>
              <ClassicalPanel speed={speed} onValue={handleClassicalValue} />
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 28 }} animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.5, delay: 0.32 }}>
              <QuantumPanel speed={speed} onValue={handleQuantumValue} />
            </motion.div>
          </div>
